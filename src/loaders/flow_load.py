...
def _choose_columns(target: str, candidates: List[str], *, interactive: bool) -> List[str]:
	...
	print(f"\nMultiple columns match '{target}':")
	for i, name in enumerate(candidates, start=1):
		print(f"  {i}) {name}")
	print("Choose columns (comma-separated numbers), or 'a' for all")
	...
	if not idxs:
		print("⚠ Invalid selection; using ALL matching columns")
		return candidates
...
def display_data(data):
	...
	print(f"{'='*80}")
	print("Note: FLOW loader keeps row count; only sorting may change order.")
 ...
 *,
 ...
 interactive: bool = True,
	...
	try:
		...
		date_cols = col_sel.get('date', []) or []
		cash_cols = col_sel.get('cash', []) or []
		...
		if output_rows != input_rows and input_rows is not None:
